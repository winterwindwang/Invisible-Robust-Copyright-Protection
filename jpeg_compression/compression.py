...
class compress_jpeg(nn.Module):
    ...
    def __init__(self, device, rounding=torch.round):
    ...
    def forward(self, image, factor):
        self.c_quantize = c_quantize(rounding=self.rounding, factor=factor).to(self.device)
        self.y_quantize = y_quantize(rounding=self.rounding, factor=factor).to(self.device)
...
class compress_jpeg_origin(nn.Module):
    """ Full JPEG compression algortihm
    Input:
        imgs(tensor): batch x 3 x height x width
        rounding(function): rounding function to use
    ...
    def __init__(self, rounding=torch.round, factor=1):
        super(compress_jpeg_origin, self).__init__()
        self.l1 = nn.Sequential(
            rgb_to_ycbcr_jpeg(),
            chroma_subsampling()
        )
