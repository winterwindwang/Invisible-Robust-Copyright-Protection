...
class decompress_jpeg(nn.Module):
    ...
    def __init__(self, height, width, device, rounding=torch.round):
    ...
    def forward(self, y, cb, cr, factor):
        self.c_dequantize = c_dequantize(factor=factor).to(self.device)
        self.y_dequantize = y_dequantize(factor=factor).to(self.device)
...
class decompress_jpeg_origin(nn.Module):
    """ Full JPEG decompression algortihm
    Input:
        compressed(dict(tensor)): batch x h*w/64 x 8 x 8
        rounding(function): rounding function to use
    ...
    def __init__(self, height, width, rounding=torch.round, factor=1):
        super(decompress_jpeg_origin, self).__init__()
